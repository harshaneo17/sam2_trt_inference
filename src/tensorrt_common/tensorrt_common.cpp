...
extern "C" void copy_to_c_build_config(const tensorrt_common::BuildConfig* src, BuildConfigC* dest)
{
    ...
    {
        strncpy(dest->debug_tensors[i],
                src->debug_tensors[i].c_str(),
                sizeof(dest->debug_tensors[i]) - 1);
...
TrtCommon::TrtCommon(const std::string& model_path,
...
void TrtCommon::setup()
{
    ...
    else if (model_file_path_.extension() == ".onnx")
    {
        ...
        {
            ext =
                "DLA" + std::to_string(build_config_->dla_core_id) + "-" + calib_name + precision_;
            if (precision_ == "int8")
            {
...
bool TrtCommon::buildEngineFromOnnx(const std::string& onnx_file_path,
                                    const std::string& output_engine_file_path)
{
    ...
    else
    {
        ...
        for (std::int32_t i = 0; i < num_input_layers; i++)
        {
            ...
            opt_prof->setDimensions(network->getInput(i)->getName(),
